...
func TestUndirected(t *testing.T) {
	...
	if hasEdge := g.HasEdge(1, 2); !hasEdge {
		t.Errorf("g.HasEdge(1,2) = %v, want: %v", hasEdge, true)
	}

	if hasEdge := g.HasEdge(0, 3); hasEdge {
		t.Errorf("g.HasEdge(0,3) = %v, want: %v", hasEdge, false)
	}
}
