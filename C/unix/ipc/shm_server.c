...
int main(int argc, char **argv) {
  ...
  // Get shared memory
  if ((fd_shm = shm_open(SHARED_MEM_NAME, O_RDWR | O_CREAT | O_EXCL, 0660)) ==
      -1)
    error("shm_open");
  ...
  // value = MAX_BUFFERS
  if ((buffer_count_sem = sem_open(SEM_BUFFER_COUNT_NAME, O_CREAT | O_EXCL,
                                   0660, MAX_BUFFERS)) == SEM_FAILED)
    error("sem_open");

  // counting semaphore, indicating the number of strings to be printed. Initial
  // value = 0
  if ((spool_signal_sem = sem_open(SEM_SPOOL_SIGNAL_NAME, O_CREAT | O_EXCL,
                                   0660, 0)) == SEM_FAILED)
    error("sem_open");
